...
pub struct Bitset<
    StorageType, // 存储类型
    const BIT_WIDTH: usize, //每个存储单元中的位数
    const SHIFT: usize, // 位移位数 计算位的存储索引
    const STORAGE_WIDTH: usize, //存储宽度
> where
    StorageType: PrimInt,
{
...
mod tests {
    ...
    #[test]
    fn test_iter_16s() {
        let mut bs = super::Bitset16::<16>::new();
        bs.set(0);
        bs.set(1);
        bs.set(2);
        bs.set(4);
        ...
        assert_eq!(v, vec![0, 1, 2, 4, 8, 16, 63, 255]);
    }
    ...
    fn test_iter_64s() {
        ...
        bs.set(255);
        let v: Vec<usize> = bs.iter().collect();
        assert_eq!(v, vec![0, 1, 2, 4, 8, 9, 10, 11, 12, 16, 34, 47, 48, 49, 127, 255]);
    }
}
