...
const REPLACE_WIDTH: usize = 2;
impl<T: Partial + PartialEq + Debug + for<'a> From<&'a [u8]>> PrefixTraits for T {}

pub(crate) struct RawTree<P: PrefixTraits, V: Clone> {
    pub(crate) root: Node<P, V>,
}

unsafe impl<P: PrefixTraits, V: Clone> Send for RawTree<P, V> {}

unsafe impl<P: PrefixTraits, V: Clone> Sync for RawTree<P, V> {}

impl<P: PrefixTraits, V: Clone> Default for RawTree<P, V> {
    fn default() -> Self {
        Self::new()
    }
}

impl<P: PrefixTraits, V: Clone> RawTree<P, V> {
    pub fn new() -> Self {
        RawTree {
    ...
    pub fn insert(&self, key: &P, value: V, guard: &Guard) -> Option<V> {
        ...
        loop {
            match RawTree::insert_inner(&self.root, key, value.clone(), 0, guard) {
                Ok(v) => return None,
                Err(e) => match e {
    ...
    ) -> Result<(), TreeError>
    {
        ...
        // 针对空节点来完成替换操作
        if next_node.node_type() == NodeType::Empty && depth == 0 {
        ...
        loop {
            ...
            if !is_prefix_match {
                node = next_node.read_lock()?;
                let n4_prefix = node.as_ref().prefix.partial_before(longest_common_prefix);
                let mut n4 = Node::<P,V>::new(NodeType::Node4,n4_prefix );
                ...
                write_node.as_mut().add_child(k1, replacement_current);
                write_node.as_mut().add_child(k2, new_leaf);
            ...
            if is_prefix_match && node.as_ref().prefix.len() == key.length_at(depth) {
                node = next_node.read_lock()?;
                let new_leaf = Node::new_leaf(key.partial_after(0), value.clone());
            ...
            let child_for_key = node.as_ref().find_child(k);
            ...
            let new_leaf = Node::new_leaf(key.partial_after(depth + longest_common_prefix), value.clone());
...
fn update_tree_stats<P: Partial + Clone, V: Copy>(tree_stats: &mut TreeStats, node: &Node<P, V>) {
    tree_stats
        .node_stats
        .entry(node.capacity())
        .and_modify(|e| {
...
mod tests {
    ...
    fn test_bulk_random_string_query() {
        ...
        let keys = gen_random_string_keys::<16>(3, 2, 3);
        print!("{}", keys.len());
        let mut num_inserted = 0;

        for (_i, key) in keys[0..20].iter().enumerate() {
            // println!("{:?}", key.1.clone() );
            ...
            println!("{}", key.1);
            tree.insert(&key.0, key.1.clone(), &guard);
        }
